...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    non_reg_cost = np.sum(np.log(selected_probabilities))
    non_reg_cost *= -1 / N
    reg_cost = np.sum(np.square(theta))
    reg_cost *= lambda_factor / 2.0
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    non_reg_gradient = np.dot(M-probabilities, X)
    non_reg_gradient *= -itemp/num_examples
    return theta - alpha * (non_reg_gradient + lambda_factor * theta)
