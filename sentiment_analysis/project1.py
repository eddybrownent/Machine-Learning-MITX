...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    h_loss = max(0.0, 1 - prediction * label)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        h_loss += hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0)
    return h_loss/ len(labels)
...
def pegasos_single_step_update(
    ...
    if label * (np.dot(feature_vector, theta) + theta_0) <= 1:
        return ((mult * theta) + (eta * label * feature_vector), (theta_0) + (eta * label))
